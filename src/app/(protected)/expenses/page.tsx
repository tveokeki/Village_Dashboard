...
export default function ExpensesPage() {
  ...
  async function createExpense(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const payload = { title: f.get("title"), description: f.get("description"), items: items.filter((it) => it.description && it.category && it.amount), notes: f.get("notes") };
      const res = await fetch(uatPath("/api/finance/expenses"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Save failed");
      setSuccess(t("ส่งคำขอเบิกเรียบร้อย", "Expense request submitted"));
      formEl.reset(); setItems([{ description: "", category: "", amount: "", payment_source: "bank_transfer", spent_at: "" }]); setOpenForm(false); await loadExpenses();
  ...
  return <div className="py-6 min-w-0">
    <FinancePageHeader title={t("รายจ่ายและคำขอเบิก", "Expenses")} description={t("สร้างคำขอเบิก อนุมัติรายจ่าย และติดตามเงินสดย่อย", "Create requests, approve expenses, and track petty cash")}>
      <button onClick={() => setOpenForm((v) => !v)} className="btn-primary w-full sm:w-auto">+ {t("ขอเบิกค่าใช้จ่าย", "New expense request")}</button>
    </FinancePageHeader>
