...
const paymentTypes = new Set(["monthly", "3_months", "6_months", "yearly"]);
...
export async function GET(req: NextRequest) {
  try {
    ...
    const rowsSql = `
      SELECT
        mf.id, mf.member_id, m.house_number, m.owner_name, m.contact_info,
      ...
      ORDER BY mf.due_date DESC, m.house_number ASC
    ...
    return NextResponse.json({ revenue: rows.rows, stats: stats.rows, limit, offset });
  } catch (err: any) {
